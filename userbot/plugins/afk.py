...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:  # pylint:disable=E0602
        ...
        msg = None
        message_to_reply = f"__Am being offline from__ `{total_afk_time}`\nWhere He Is: ~~Enjoying With some Ladies In Tub~~ " + \
            f"\n\n__I promise I'll back suddenly after CORONA leaves.__\n**REASON**: {reason}" \
            if reason \
            else f"**YOU NUB**\n__I am offline now. Since when, you ask? For {total_afk_time} I guess.__\n\nWhen will I be back? ~~Soon~~ __When Corona leaves**(☣)**  "
